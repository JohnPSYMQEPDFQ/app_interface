...
#
    ...
    def deep_flatten_to_h( separator: [], sort_TF: false, _parent_key: nil, _flattened_H: {} )
#
...
#   separated by 'separator'.  
        ...
        self.each_with_index do | thingy, idx |
            ...
            if value.not_nil? && value.respond_to?( :each_with_index ) 
                value.deep_flatten_to_h( separator:   separator, 
                                           sort_TF:     sort_TF,
    ...
    alias_method :nested_flatten_to_h, :deep_flatten_to_h

    def to_composite_key_h
        return self if ( ! self.respond_to?( :each_with_index ) )
        
        get_k_v = lambda { | parent_obj, item, idx | 
            case true
            when parent_obj.is_a?( Hash) && item.is_a?( Array )                                            
        ...
        new_hash = {}
        self.each_with_index do | outer_item, outer_idx |
            outer_key, outer_value = get_k_v.( self, outer_item, outer_idx )      
            if ( ! outer_value.respond_to?( :each_with_index ) ) then
                new_hash[ [ outer_key ] ] = outer_value
                next
            end
           #SE.q {['self.class','outer_key','outer_value']}
            outer_value.each_with_index do | inner_item, inner_idx |
               ...
               #SE.q {['outer_value.class','inner_key','inner_value']}
                if ( inner_key.is_not_a?( Array ) ) then
                    raise "Was expecting inner_key to be an 'Array', not #{inner_key}!"
                end
