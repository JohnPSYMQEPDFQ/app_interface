...
class Separator_Punctuation
    ...
    def initialize
        ...
        self.usable_text_punct_chars = self.all_punct_chars.dup
    end
    def reserve_punctuation_chars( separator_RE )
        ...
        if ( not self.all_punct_chars.match?( separator_RE ) ) then
            SE.puts "#{SE.lineno}: No punctuation characters found for param '#{separator_RE.to_s}'"
            raise
        end
        all_punct_chars_temp = self.all_punct_chars.dup
        arr = [ ]
        loop do
            match_O = self.usable_text_punct_chars.match( separator_RE )
            if ( match_O ) then
                arr << match_O[ 0 ]
                self.usable_text_punct_chars[ match_O[ 0 ] ] = ''
