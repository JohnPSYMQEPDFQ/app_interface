...
def get_location_title( loc_data_H )
    ...
    if ( xyz_A.maxindex == 3 ) then        
        if ( xyz_letter_A[ 3 ].next >= xyz_letter_A[ 2 ] )
            stringer.concat( "-#{xyz_A[ 3 ]}" )
        else
            SE.puts "#{SE.lineno}: =============================="
            SE.puts "NOT (xyz_letter_A[ 3 ] >= xyz_letter_A[ 2 ] )"
            SE.q {'loc_id'}
            SE.q {'xyz_A'}
            SE.q {'xyz_letter_A'}
            raise            
...
def set_location( loc_data_H, container_loc_uri_H )
    location_title = get_location_title( loc_data_H ) 
    loc_data_H[ CONTAINER_LOC_TITLE ] = location_title
    if ( container_loc_uri_H.has_key?( location_title ) ) then
        loc_data_H[ CONTAINER_LOC_URI ] = container_loc_uri_H[ location_title ].dup
        return                            container_loc_uri_H[ location_title ].dup
    else
        search_text = %Q|title:"#{location_title}"|         # Note field-name and quotes in text
        loc_record_H_A = self.rep_O.search( record_type_A: [ K.location ], search_text: search_text ).record_H_A
        if ( loc_record_H_A.nil? || loc_record_H_A.empty? ) then
            loc_data_H[ CONTAINER_LOC_URI ]       = CREATE_ALA_LOCATION
            container_loc_uri_H[ location_title ] = CREATE_ALA_LOCATION           
    ...
    container_loc_uri_H[ location_title ] = loc_record_H.fetch( K.uri ).dup
    loc_data_H[ CONTAINER_LOC_URI ]       = container_loc_uri_H[ location_title ].dup
    return                                  container_loc_uri_H[ location_title ].dup
end
...
def print__aoz_having_locations( source_data_H__BY_ao_uri )
    captured_stringer = capture_stdout do
        puts ''
        puts "AO's having locations:"
        source_data_H__BY_ao_uri.each_pair do | _, source_data_H |
            puts "#{source_data_H.fetch( K.level )}: #{source_data_H.fetch( K.title )[ 0,60 ]} " +
                 "`#{source_data_H.fetch( K.uri ).trailing_digits}` " +
                 ''                   
            puts "        Text: #{source_data_H[ LOC_DATA_H ][ TEXT_H ][ TEXT_VALUE ].gsub(/([\r]*[\n])+/, " | ")}"       
...
def print__aoz_with_unused_locations( source_data_H__BY_ao_uri, unused_ao_location_A )
    captured_stringer = capture_stdout do
        puts ''
        puts "AO's having locations that were never used:"
        unused_ao_location_A.each do | unused_uri |
            source_data_H = source_data_H__BY_ao_uri.fetch( unused_uri )
            puts "#{source_data_H.fetch( K.level )}: #{source_data_H.fetch( K.title )[ 0,60 ]} " +
                 "`#{source_data_H.fetch( K.uri ).trailing_digits}` " +
                 ''             
            puts "        Text: #{source_data_H[ LOC_DATA_H ][ TEXT_H ][ TEXT_VALUE ].gsub(/([\r]*[\n])+/, " | ")}"
...
def print__tcz_locations( tc_data_H__BY_tc_uri, tc_QO, ao_QO )    
    ...
    tc_data_H__BY_tc_uri.each_pair do | tc_uri, tc_data_H |
        ...
        arr = tc_data_H__BY_tc_uri.fetch( tc_uri ).fetch( PICKED_LOC_DATA_H_A )
            .map { | picked_loc_data_H |  picked_loc_data_H.fetch( LOC_DATA_H ).fetch( LOC_ID ) } 
            .uniq
        ...
        tc_data_H.fetch( PICKED_LOC_DATA_H_A ).each do | picked_loc_data_H |            
            loc_data_H = picked_loc_data_H.fetch( LOC_DATA_H )
            loc_id = loc_data_H.fetch( LOC_ID )
            ...
            picked_loc_data_H[ CHILD_H_A ].each_pair do | picked_ao_uri, child_H_A |            
                ...
                if ( loc_id.in?( NO_LOCATION, INVALID_LOCATION ) ) then
                   ...
                   #       ' ' + "#{ao_record_H.fetch( K.level )}: #{ao_record_H.fetch( K.title )[ 0,60 ]}" + 
                    ...
                    puts text
                       ...
                       #SE.q {'picked_loc_data_H[ CHILD_H_A ].keys'}
                       #SE.q {'tc_data_H'}
                       #raise
                   #end
...
def print__tcz_by_type_indicator( tc_data_H__BY_tc_type_indicator )
    puts ''
    tc_data_H__BY_tc_type_indicator.each_pair do | type_N_indicator, data_H |
        next if data_H[ K.hierarchy ].length <= 1
        puts "Multiple hierarchies for '#{type_N_indicator}', AO's: #{data_H[ AO_CNT ]}, Location count: #{data_H[ K.hierarchy ].length}"
    end
    puts ''
end
...
def strip_off_more_noise( param_1 )
    ...
    result.sub!( /\A\s*(SMCC|Statewide\s+Museum\s+Collections?\s+Center)[,;:]?/i, ' ' )
    result.sub!( /\A\s*rg\s+\d+(\s+series\s+\d+[.]?)\s*/i, ' ' )
    result.strip!
    return result
end
...
def get_loc_from_ao( record_H )
    ...
    record_H.to_CKA_h.each_pair do | composite_key_A, source_text_value |
        ...
        ld_1 = SE::Loop_detector.new( 1000 )
        loop do   
            ...
            if source_data_H.empty? then
               ...
               source_data_H[ TEXT_H ]  = { TEXT_KEY_CKA => composite_key_A,
                                            TEXT_VALUE   => source_text_value   # This HAS the CR/LF !!!! 
...
#                   break
            ...
            if container_FOOD.not_blank? then                       
                ...
                SE.q {'record_H.fetch( K.title )'}
                SE.q {"composite_key_A.join( ',' )"}
                SE.q {'source_text_value_WO_CRLF[ 0, 40 ]'}
                SE.q {'container_FOOD[ 0, 40 ]'}
                SE.q {'location_FOOD[ 0, 40 ]'}
                SE.q {'source_data_H'}
                next  
            end
        end
    ...
    if cnt > 1 then
        puts 'More than one location, without any box-ranges, found for record:'
        puts "#{record_H.fetch( K.level )}: #{record_H.fetch( K.title )} " +
             "`#{record_H.fetch( K.uri ).trailing_digits}` " +
             ''
...
BEGIN {    
    ...
    INVALID_LOCATION_A           = :INVALID_LOCATION_A
    ...
    PICKED_LOC_DATA_H_A          = :PICKED_LOC_DATA_H_A
    TEXT_H                       = :TEXT_H    
...
OptionParser.new do | option |
    ...
    option.on( "--irl", "--ignore-res-locations", "Ignore the Resource locations." ) do | opt_arg |
        self.cmdln_option_H[ :use_res_locs ] = false
    end
...
SE.q {'source_data_H__BY_ao_uri.length'}

SE.puts "#{source_data_H__BY_ao_uri.length} AO's with locations."
if ( source_data_H__BY_ao_uri.empty? ) then
    puts "Nothing to do."
    exit
end
...
#           throw :done if ( tc_record_H.fetch( K.indicator ).downcase == 'unk' ) 
            hierarchy_uri_A.each_with_index do | hierarchy_uri, hierarchy_idx |     
                ...
                if source_data_H__BY_ao_uri.has_key?( hierarchy_uri ) 
                    ...
                    source_data_H      = source_data_H__BY_ao_uri.fetch( hierarchy_uri )
                    ...
                    loc_id_n_range_H_A.each do | loc_id_n_range_H | 
                        ...
                        if ( picked_data_H.fetch( LOC_DATA_H ).empty? ) then
                            ...
                            arr = hierarchy_uri_A[ hierarchy_idx + 1, hierarchy_uri_A.length ]
                            if ( tc_data_H__BY_tc_type_indicator[ type_N_indicator ][ K.hierarchy ].not_include?( arr ) ) then
                                tc_data_H__BY_tc_type_indicator[ type_N_indicator ][ K.hierarchy ] << arr
                            end
                            case
                           ...
                           #SE.q {'picked_data_H'} if ( $DEBUG )
                            ...
                            if ( picked_data_H.fetch( LOC_DATA_H ).fetch( LOC_RANGE_H ).not_empty? &&
                               ( range_H_A.nil? || range_H_A.empty? ) ) then
                                next   # If we already HAVE a picked range location, then a new non-range (nil) location, 
                                       # or new non-match-ranch (empty) location can be ignored.
                            end
                            if ( picked_data_H.fetch( LOC_DATA_H ).fetch( LOC_RANGE_H ).empty? &&
                               ( range_H_A.nil? || range_H_A.empty? ) ) then
                                if ( picked_data_H[ K.uri ] != hierarchy_uri ) then                                
                                    next   # If we already have a picked non-range location, and we hit a new non-range (nil) location
                                           # or new non-match-ranch (empty) location higher up the food chain, we can ignore it.

                                    # However, if it's at the same hierarchy, this means we have two non-range
                                    # locations from the same source (which is invalid).
                                end
                            end                        
                            ...
                            if ( picked_data_H[ LOC_DATA_H ][ LOC_ID ] == INVALID_LOCATION ) then
                                ...
                                if ( picked_data_H.fetch( LOC_DATA_H ).fetch( LOC_RANGE_H ).not_empty? ) then
                                    ...
                                    picked_data_H[ LOC_DATA_H ][ INVALID_LOCATION_A ] = [ MULTIPLE_MATCHES, ON_RANGES ]
                                else
                                    picked_data_H[ LOC_DATA_H ][ INVALID_LOCATION_A ] = [ MULTIPLE_MATCHES, ON_LOCATIONS ]
                                end
...
#                           puts "#{SE.lineno}: =============================="
                            ...
                            print "SOURCE AO URI's: `#{picked_data_H.fetch( K.uri ).trailing_digits}`, `#{hierarchy_uri.trailing_digits}` "
                            print "TC URI: `#{tc_record_H.fetch( K.uri ).trailing_digits}`, #{type_N_indicator}"
                            puts ''
                            puts "   Note: #{error_note}"
                            puts ''
                           #SE.q {'picked_data_H'} if ( $DEBUG )
...
#               puts "#{SE.lineno}: =============================="
                ...
                set_location( picked_data_H[ LOC_DATA_H ], container_loc_uri_H ) 
        ...
        tc_data_H__picked_loc_data_H = tc_data_H.fetch( PICKED_LOC_DATA_H_A ).find { | picked_loc_data_H | 
            picked_loc_data_H.fetch( LOC_DATA_H ) == picked_data_H.fetch( LOC_DATA_H ) } || begin                               
                tc_data_H[ PICKED_LOC_DATA_H_A ] << { LOC_DATA_H  => picked_data_H.fetch( LOC_DATA_H ),
                                                      CHILD_H_A   => {}, 
                                                     }                                                           
                tc_data_H[ PICKED_LOC_DATA_H_A ].last    # This is needed because the '<<' OR '.push" returns the entire array                                          
            end
        SE.q {'tc_data_H__picked_loc_data_H'} if ( $DEBUG )
        ...
        tc_data_H__picked_loc_data_H[ CHILD_H_A ][ picked_data_H.fetch( K.uri ) ] ||= []
                 
        new_child_H = { K.uri      => ao_uri,
                        K.instance => instance_idx,
                       }.deep_yield { | y | y.freeze }  
        bool = tc_data_H__picked_loc_data_H.fetch( CHILD_H_A )
        ...
        picked_ao_uri = picked_data_H[ K.uri ]
        tc_data_H__picked_loc_data_H[ CHILD_H_A ][ picked_ao_uri ] << new_child_H 
        if ( picked_data_H_A__BY_ao_uri.has_key?( picked_ao_uri ) ) 
            if ( picked_data_H_A__BY_ao_uri[ picked_ao_uri ] != picked_data_H ) 
                SE.q {'picked_data_H'}
                SE.q {'picked_ao_uri'}
                SE.q {'picked_data_H_A__BY_ao_uri[ picked_ao_uri ]'}
                SE.raise
            end
        else
            picked_data_H_A__BY_ao_uri[ picked_ao_uri ] = picked_data_H          
        end
...
tc_to_skip_A = []
tc_data_H__BY_tc_uri.each_pair do | tc_uri, tc_data_H |
    picked_loc_data_H_A = tc_data_H.fetch( PICKED_LOC_DATA_H_A )
    if ( picked_loc_data_H_A.empty? ) then
        SE.puts "#{SE.lineno}: =============================="
        SE.puts "tc_data_H.fetch( PICKED_LOC_DATA_H_A ).empty?"
        SE.q {['tc_uri', 'tc_data_H']}
        raise
    end

    cnt = picked_loc_data_H_A.count { | picked_loc_data_H | 
                                    picked_loc_data_H.fetch( LOC_DATA_H )
                                                 .fetch( LOC_ID )
    ...
    if ( cnt > 0 ) then #We have an actual location, so delete the NO_LOCATION location.   A NO_LOCATION
        ...
        picked_loc_data_H_A.map! do | picked_loc_data_H |
            loc_id = picked_loc_data_H.fetch( LOC_DATA_H ).fetch( LOC_ID )
            if ( loc_id.in?( NO_LOCATION ) ) then
               ...
               picked_loc_data_H = {}     #THIS WORKS, ONLY because of 'next picked_loc_data_H' below.
            end
            next picked_loc_data_H        #map! is expecting something back for EACH iteration of the loop. 
                                      #Without 'next picked_loc_data_H', the value of the last statement is the  
                                      #return value; which is the if statement IF IT'S FALSE.
        end
    end
    picked_loc_data_H_A.delete_if { | picked_loc_data_H | picked_loc_data_H.empty? }
    next if ( picked_loc_data_H_A.empty? )

    arr = tc_data_H__BY_tc_uri.fetch( tc_uri ).fetch( PICKED_LOC_DATA_H_A )
                                              .map { | picked_loc_data_H |  picked_loc_data_H.fetch( LOC_DATA_H ).fetch( LOC_ID ) } 
                                              .uniq
    if ( arr.length > 1 ) then
        picked_loc_data_H_A << { LOC_DATA_H => { LOC_ID      => DUPLICATE_LOCATIONS,      
                                                 LOC_RANGE_H => {},
                                                },
                                 CHILD_H_A => {},              
                                 K.uri     => nil,                                            
                                }.deep_copy      
        set_location( picked_loc_data_H_A.last[ LOC_DATA_H ], container_loc_uri_H ) 
        puts "#{tc_data_H.fetch( K.type )} #{tc_data_H.fetch( K.indicator )} `#{tc_uri.trailing_digits}` Added '#{ERROR_LABEL}: #{DUPLICATE_LOCATIONS}'"
        puts ''
    end
    
    cnt = picked_loc_data_H_A.count { | picked_loc_data_H | picked_loc_data_H.fetch( LOC_DATA_H ).fetch( LOC_ID ) == NO_LOCATION }                                                                           
    if ( cnt > 1 ) then
        SE.puts "#{SE.lineno}: =============================="
    ...
    cnt = picked_loc_data_H_A.count { | picked_loc_data_H | picked_loc_data_H.fetch( LOC_DATA_H ).fetch( LOC_ID ) == INVALID_LOCATION }                                                                           
    if ( cnt > 1 ) then
       #SE.puts "#{SE.lineno}: =============================="
    ...
    cnt = picked_loc_data_H_A.count { | picked_loc_data_H | picked_loc_data_H.fetch( LOC_DATA_H ).fetch( LOC_ID ) == DUPLICATE_LOCATIONS }                                                                           
    if ( cnt > 1 ) then
        SE.puts "#{SE.lineno}: =============================="
...
tc_data_H__BY_tc_uri.each_pair do | tc_uri, tc_data_H |
    next if ( tc_data_H.fetch( PICKED_LOC_DATA_H_A ).empty? )
    tc_data_H.fetch( PICKED_LOC_DATA_H_A ).each do | picked_loc_data_H |
        loc_data_H = picked_loc_data_H.fetch( LOC_DATA_H )
        ...
        picked_ao_uri_A.concat( picked_loc_data_H.fetch( CHILD_H_A ).keys )
...
loc_data_H_A__BY_new_ALA_location_H.each_pair do | new_ALA_location_key_H, loc_data_H_A | 
...
#   Delete the NO_LOCATION link if present. If there's still no location, it will be added back.
    ...
    tc_data_H.fetch( PICKED_LOC_DATA_H_A ).each do | picked_loc_data_H |  
        ...
        picked_loc_data_H[ CHILD_H_A ].each_key do | picked_ao_uri |
            if ( picked_note.not_blank? && picked_ao_uri[ 0 ] == '/' ) then
                ...
                begin
                    ...
                    SE.q {'picked_data_H_A__BY_ao_uri'}
                    SE.q {'picked_loc_data_H[ CHILD_H_A ]'}
                    raise
                end
                begin
                ...
                if ( picked_loc_text != picked_data_H_A__BY_ao_uri[ picked_ao_uri ][ TEXT_H ].fetch( TEXT_VALUE ) )
                    SE.q {'picked_loc_text'}
                    SE.q {'picked_data_H_A__BY_ao_uri[ picked_ao_uri ]'}
                    SE.q {'picked_loc_text_CKA'}
                    SE.q {'picked_ao_uri'}
                    SE.q {'ao_BO.record_H'}
