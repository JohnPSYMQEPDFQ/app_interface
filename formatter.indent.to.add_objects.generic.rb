...
def put_indent( level_number_A, level_title_A )
    ...
    when 0
        ...
        raise
    when 1 .. $cmdln_option_G[ :max_series ]
...
$cmdln_option_G = { :min_group_size => 10, 
                    :max_series => 0,
                    :max_levels => nil,
                    :r => nil }
OptionParser.new do |option|
    option.banner = "Usage: #{myself_name} [options] [file]"
    option.on( "-g n", "--min-group-size n", OptionParser::DecimalInteger, "Min records in a Series/Subseries/Record-group (default = 10)" ) do |opt_arg|
        $cmdln_option_G[ :min_group_size ] = opt_arg
    end
    option.on( "-s n", "--max-series n", OptionParser::DecimalInteger, "Max number of Series/Subseries (default = 0)" ) do |opt_arg|
