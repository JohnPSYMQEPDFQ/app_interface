...
module Date_Clumps_parse_text_dates
    ...
    def do_find( )   
        ...
        self.date_clump_C = 
            Struct.new( :full_match_string__everything,
                ...
                def judge_date( judgement, input_error_msg, print = true )    
                    ...
                    SE.puts ''              if ( print )
                    self.error_msg  = +''   if ( self.error_msg.nil? )
                    self.error_msg << '  '  if ( self.error_msg.not_blank? )
                    self.error_msg << input_error_msg
                    return if ( judgement.nil? )
                    if ( self.morality.nil? ) then
                        self.morality = judgement
                        return
                    end
                    if ( self.morality == :bad ) then
                        return if ( judgement == :bad )
                        SE.puts "#{SE.lineno}: Date morality was already bad, not changed to #{judgement}"
                        return
                    end
                    SE.puts "#{SE.lineno}: Date morality was already #{self.morality}, changed to #{judgement}"
                    self.morality = judgement
                    return
                end
                def date_match_S__from
                    if ( self.date_match_S__A.length > 2 ) then
                        SE.puts "#{SE.lineno}: I shouldn't be here: date_clump_S.length > 2"
                        SE.q { 'self' }
                        raise
                    end
                    return self.date_match_S__A[ 0 ]
                end
                def date_match_S__thru
                    if ( self.date_match_S__A.length > 2 ) then
                        SE.puts "#{SE.lineno}: I shouldn't be here: date_clump_S.length > 2"
                        SE.q { 'self' }
                        raise
                    end
                    return self.date_match_S__A[ 1 ]
                end      
                def as_from_date
                    if ( date_match_S__from.nil? ) then
                        SE.puts "#{SE.lineno}: I shouldn't be here: from == nil?"
                        SE.q { 'self' }
                        raise
                    end
                ...
                def as_date_expression
                    if ( self.aspace_O.nil? ) then
                        SE.puts "#{SE.lineno}: This method needs an aspace_O param when doing a 'Find_Dates_in_String.new'"
                        raise
                    end
                    return self.aspace_O.format_date_expression( from_date: as_from_date, 
                                                                 thru_date: as_thru_date, 
                                                                 certainty: certainty )                                                
                end
            end
                        
        self.possible_date_C = 
            Struct.new( :pattern_name,
                        :regexp,
                        :match_O,
                       )
        ...
        self.date_match_C =
            Struct.new( :match_O,
                ...
                def date_modifier__certainty__xlat
                    return ''            if ( match_O.named_captures[ 'date_modifier' ].nil? )
                    return K.approximate if ( match_O.named_captures[ 'date_modifier' ].match?( /^(circa|ca([.])?)/i ) )
                    return ''
                end
                def date_modifier_bulk?
                ...
                def all_pieces
                    return  match_O.named_captures[ 'begin_M' ] +
                            match_O.named_captures[ 'date_M' ] +    # Includes 'date_modifier'
...
#       SE.q {[ 'param__initial_offset', 'self.output_data_O.string' ]}
        ...
        self.date_pattern_RES_S__A.each do | date_pattern_RES_S |
            ...
            if ( param__looking_for_a_thru_date ) then
                regexp = %r{(?<begin_M>#{self.thru_date_begin_delim_RES})(?<date_M>#{date_pattern_RES_S.pattern_RES})(?<end_M>#{self.thru_date_end_delim_RES})}xi
            else
                regexp = %r{(?<begin_M>#{self.begin_delim_RES})(?<date_M>#{self.date_modifier_RES}#{date_pattern_RES_S.pattern_RES})(?<end_M>#{self.end_delim_RES})}xi               
            end
            scan_begin_offset = param__initial_offset + 0
            ld = SE::Loop_detector.new( 100 )
            loop do   
                ld.loop
                break if ( scan_begin_offset >= self.output_data_O.string.maxoffset )
                match_O = self.output_data_O.string.match( regexp, scan_begin_offset )
                break if ( match_O.nil? )
              # SE.q {[ 'scan_begin_offset', 'self.output_data_O.string.maxoffset', 'self.output_data_O.string' ]}
              # SE.q {[ 'match_O' ]}
                ...
                if ( match_O.named_captures[ 'end_M' ].match?( /#{self.thru_date_separator_RES}/ix ) ) then
                    result = get_tree_of__possible_date_S__A_A( scan_begin_offset, true, param__level + 1 )
                    tree_of__possible_date_S__A_A << [ self.possible_date_C.new( pattern_name, regexp, match_O ), result ]
                else
                    tree_of__possible_date_S__A_A << [ self.possible_date_C.new( pattern_name, regexp, match_O ), [ ] ]
                end
            end
        end
...
#       Return only the longest date ( element 0 after sorting) of all the dates found.
    ...
    def date_clumps_parse_text_dates( )
        ...
        loop do   
            ld.loop
            if ( self.option_H[ :debug_options ].include?( :print_output_data )) then
                SE.puts ''
...
#           date_match_S__A is the from date [element 0] and (optional) thru date [element 1].  
            ...
            date_clump_S = self.date_clump_C.new( full_match_string__everything: +'',
                                                  full_match_string__begin_delim__lstrip: +'', 
                                                  date_match_string: +'',
                                                  full_match_string__end_delim__rstrip: +'',
...
#                    First date or only date
                if ( date_match_idx == 0 ) 
                    date_clump_S.uid                                                   = self.date_clump_uid_O.uid_of_num( self.date_clump_S__A.length )                    
                    date_clump_S.date_match_string__beginning_offset                   = date_match_S.match_O.offset( :date_M )[ 0 ]  
                    date_clump_S.certainty                                             = date_match_S.date_modifier__certainty__xlat
                    date_clump_S.bulk_TF                                               = date_match_S.date_modifier_bulk?
                    date_match_string                                                 << date_match_S.piece( 1 )   # includes 'date_modifier'
...
#                   This happens with "run-on" thru dates, eg: 1970 - 1980 - 1990
...
#                   Last date                
                if ( date_match_idx > 0 and date_match_idx == date_clump_S.date_match_S__A.maxindex ) then
                    ...
                    if ( not ( stringer.length > 1 and 
                               stringer.match( /#{self.end_delim_RES}/ ) and
                               full_match_string__begin_delim__lstrip[ 0, 1 ].match( /#{self.begin_delim_RES}/ ) 
                              ) 
                         ) then
...
#           SE.q {[ 'date_clump_S' ]}                     
            self.output_data_O.string[ date_clump_S.full_match_string__begin_delim__lstrip__adj_offset, 
